...
import static targetModel.TargetLine.getDistanceFromLine;
import static targetModel.TargetLine.getLine;
    ...
    public double getLineLossScore(String pid, TargetLine[] lines) {
        ...
        for (Stroke stroke : sketch.strokes) {
            for (int i = 0;i < stroke.dots.size() - 1; i++) {
                ...
                double distance1 = Math.abs(getDistanceFromLine(dot1, line) - TOLERANCE);
                double distance2 = Math.abs(getDistanceFromLine(dot2, line) - TOLERANCE);
                if (distance1 <= TOLERANCE && distance2 <= TOLERANCE) {
                    if (line.isVertical()) {
