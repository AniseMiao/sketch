...
public class IdUtil {
    ...
    public static int getNum(String keyword) {
        ...
        int ret = (int)session.createQuery("select lastid from IdPO where keyword = ?1 ")
    ...
    public static Date getLastDate(String keyword) {
        Session session = HibernateUtil.getSession();
        Transaction transaction = session.beginTransaction();
        Date ret = (Date)session.createQuery("select lasttime from IdPO where keyword = ?1 ")
                .setParameter(1, keyword).uniqueResult();
        transaction.commit();
        session.close();
        return ret;
    ...
    public static void updateNum(String keyword) {
        Session session = HibernateUtil.getSession();
        Transaction transaction = session.beginTransaction();
        session.createQuery("update IdPO set lastid = lastid + 1 where keyword = ?1 ")
    ...
    public static void resetNum(String keyword) {
        Session session = HibernateUtil.getSession();
        Transaction transaction = session.beginTransaction();
        session.createQuery("update IdPO set lastid = 1 where keyword = ?1 ")
                .setParameter(1, keyword).executeUpdate();
        transaction.commit();
        session.close();
    }
